...
void generateDotFile(const Graph& graph, const string& filename, bool isDirectedWeighted) {
    ...
    if (dotFile.is_open()) {
        if (isDirectedWeighted) {
            ...
            for (int i = 0; i < graph.adjacencyMatrix.size(); ++i) {
                for (int j = i + 1; j < graph.adjacencyMatrix[i].size(); ++j) {
                    if (graph.adjacencyMatrix[i][j] != 0) {
                        dotFile << i << " -- " << j << " [label=\"" << graph.adjacencyMatrix[i][j] << "\"];" << endl;
                    }
                }
        ...
        cerr << "Error: No se pudo abrir el archivo DOT para escritura." << endl;
    }
}
    ...
    if (dotFile.is_open()) {
        ...
        if (checkFile.is_open()) {
            ...
            if (system("dot -Tpng floyd_warshall.dot -o floyd_warshall.png") == 0) {
                ...
                system("start floyd_warshall.png");
